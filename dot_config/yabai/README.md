...
- **Arc window pinning (Hammerspoon + yabai):** The two main Arc browser windows are pinned one-to-`main`, one-to-`school` (title-independent — it doesn't matter which goes where). The hard part: **Little Arc** popups are byte-identical to main windows in *every* yabai field (subrole, floating, even title is a page title), so yabai alone cannot tell them apart. The only reliable discriminator is `AXIdentifier` (`bigBrowserWindow-*` vs `littleBrowserWindow-*`), which **yabai cannot read** — but **Hammerspoon can**. So:
  - `~/.hammerspoon/init.lua` (chezmoi: `dot_hammerspoon/init.lua`) watches Arc windows; when one appears it reads `AXIdentifier` and, if it's a **Little Arc / PiP popup, floats it** in yabai (and shrinks it to a centered popup) so it never tiles or gets mistaken for a main window. macOS AX only exposes current-Space windows, but a popup is always created on the current Space, so it's caught at creation.
  - `yabai_arc_pin.sh` then pins the **non-floating** Arc windows (= the real main windows, now that popups float) to `main`/`school`, one each, **stably** (never disturbs a correctly-placed window; recovers drift) and **fullscreen-safe** (a fullscreen Arc window — e.g. video — is left alone and reachable via `hyper+3-9`, returning to its space on exit). It's triggered by Hammerspoon (after floating popups), not by the refresh.
  - **Dependency:** Hammerspoon must be running (set to launch at login via `hs.autoLaunch(true)`). If it's not running, Little Arc popups will tile again and main windows won't auto-pin — graceful degradation, not breakage. This replaced the old fragile title-based pinning in `yabai_workspace_refresh.sh`.
