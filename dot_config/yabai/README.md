...
- **WezTerm native fullscreen with the external connected.** The `terminal`-follow + reorder behavior (`yabai_terminal_follow.sh`) is verified only on the single built-in display. Untested with a second display: (a) fullscreen WezTerm, then `hyper+\` push to the external — does the follow hook keep the `terminal` label on it and reorder sanely given the external's reserved scratch space? (b) does `hyper+\`` (focus terminal) still reach a WezTerm that's fullscreen on the external? **How to test:** with the external connected, fullscreen WezTerm, push it over, switch around, and confirm the label tracks it and focus reaches it from both displays. The reorder reserves the first space of a non-master display as scratch, so a fullscreen terminal landing there is the main thing to watch.
- **WezTerm auto-fullscreen on startup.** The `gui-startup` → `toggle_fullscreen()` change is config-parse-validated but NOT verified on a real launch (the Claude session that built it runs *inside* WezTerm via tmux, so WezTerm wasn't restarted). **How to test:** quit and reopen WezTerm (tmux session survives) — it should come up in native fullscreen, labeled `terminal`, at position 1. If `gui_window()` is nil at startup (a known WezTerm timing edge case) it would fail to fullscreen; the fallback would be to fullscreen it from yabai's `window_created` signal instead.