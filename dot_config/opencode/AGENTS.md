...
Constraints (enforced by regex `^\s*CHEZMOI_COMMIT_OK=1\s+`, defined in
`dot_config/opencode/plugins/chezmoi-guard.ts` — keep them in sync):

- Must be the **first token** on the command line. `export … && git commit`
  won't satisfy the guard. `cd /tmp && CHEZMOI_COMMIT_OK=1 git commit` won't
  either — split into separate bash invocations or use `git -C` instead.
